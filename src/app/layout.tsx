...
async function getFooterCreditText(config: Awaited<ReturnType<typeof getConfig>>): Promise<string | undefined> {
  ...
  const footerBlock = body.find(
    (b) => ['footer', 'section_footer'].includes(String(b?.component ?? '').toLowerCase())
  );
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const navLinks = normalizeNavLinks(header?.nav_links as { href?: unknown; link?: unknown; id?: string; label?: string }[] | undefined);
