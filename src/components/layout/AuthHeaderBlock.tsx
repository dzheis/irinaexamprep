  ...
  if (session?.user?.email) {
    ...
    return (
      <div className={`flex items-center gap-2 min-w-0 ${className}`}>
        ...
        <Tooltip label="Выйти">
          <button type="button" onClick={handleLogout} className={AUTH_ICON_BTN} aria-label="Выйти">
            <svg
  ...
  return (
    <div className={`flex items-center gap-2 min-w-0 ${className}`}>
      <Tooltip label="Log in">
        <Link href="/login" className={`${AUTH_ICON_BTN} no-underline`} aria-label="Log in">
          <svg
            className={AUTH_ICON_GLYPH}
            fill="none"
            viewBox="0 0 24 24"
      ...
      <Tooltip label={SIGNUP_TOOLTIP_LABEL}>
        <Link href="/signup" className={`${AUTH_ICON_BTN} no-underline`} aria-label="Sign up">
          <svg
            className={AUTH_ICON_GLYPH}
            fill="none"
            viewBox="0 0 24 24"
