...
export async function getFreeResourcesFromStoryblok(): Promise<{
  ...
  const resources = list
    .map((item, index) => {
      ...
      const fileUrl = typeof file === 'object' && file?.filename ? String(file.filename).trim() : '';
      const downloadFilename = (item.download_filename ?? c?.download_filename)?.toString()?.trim()
